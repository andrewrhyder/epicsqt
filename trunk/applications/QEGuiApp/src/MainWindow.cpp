...
MainWindow::MainWindow(  QEGui* appIn, QString fileName, QString title, QString customisationName, bool openDialog, QWidget *parent )  : QMainWindow( parent )
{
    app = appIn;
    ...
    {
        QEForm* gui = createGui( fileName, title, customisationName ); // A profile should have been published before calling this constructor.
...
void MainWindow::loadGuiIntoCurrentWindow( QEForm* gui, bool resize )
{
    ...
    // QE_CAQTDM to be processed by QEGuiApp.pro
        ...
        caQtDMLib= new CaQtDM_Lib(this, "", "" ,mutexKnobData, 0, false, gui);
...
QWidget* MainWindow::launchGui( QString guiName, QString title, QString customisationName, QEActionRequests::Options createOption, bool hidden )
{
    ...
    switch( createOption )
    {
        ...
        case QEActionRequests::OptionOpen:
            {
                QEForm* gui = createGui( guiName, title, customisationName, false );  // Note, profile should have been published by signal code
                loadGuiIntoCurrentWindow( gui, true );
                return this;
            }
        ...
        case QEActionRequests::OptionNewTab:
            {
                ...
                QEForm* gui = createGui( guiName, title, customisationName );  // Note, profile should have been published by signal code
                if( gui )
                {
                    // If not using tabs, start tabs and migrate any single gui to the first tab
                    if( !usingTabs )
        ...
        case QEActionRequests::OptionNewWindow:
            {
                MainWindow* w = new MainWindow( app, guiName, title, customisationName, true ); // Note, profile should have been published by signal code
                w->show();
                return w;
            }
        ...
        case QEActionRequests::KindOpenFiles:
            {
                ...
                for( int i = 0; i < windows.count(); i ++ )
                {
                    ...
                    QWidget* w = mw->launchGui( window->uiFile, window->title, window->customisationName, window->creationOption, window->hidden );
...
QEForm* MainWindow::createGui( QString fileName, QString title, QString customisationName, QString restoreId, bool isDock )
{
