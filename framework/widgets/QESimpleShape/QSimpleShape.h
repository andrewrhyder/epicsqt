...
public:
    ...
    enum TextFormats { FixedText,     ///< Use user specified fixed text (default)
                       UseStrings };  ///< Use specied enumeration values, value (modulo 16) used to select item
    Q_ENUMS (TextFormats)
    ...
    // Group various colous together.
    // There order here is the order in designed.
    //
    ...
    /// Colour when deemed invalid - default is orangy.
